...
pt_tmp=pd.read_excel("JSP_dataset2.xlsx",sheet_name="Processing Time",index_col =[0])
ms_tmp=pd.read_excel("JSP_dataset2.xlsx",sheet_name="Machines Sequence",index_col =[0])
...
for n in range(num_iteration):
    ...
    for m in range(len(offspring_list)):
        ...
        if mutation_rate >= mutation_prob:
            m_chg=list(np.random.choice(num_gene, num_mutation_jobs, replace=True)) # chooses the position to mutation
